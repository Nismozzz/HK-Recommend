...
def parse_message(message: str, default_location: str | None = None) -> dict[str, Any]:
    ...
    return {"date": date, "area": area, "location_query": resolved_location or area_name, "budget": budget, "cuisine": cuisine, "food_query": food_query, "dishes": dishes, "duration": duration, "preferred_period": period, "understood": understood}
...
def deterministic_agent(message: str) -> dict[str, Any]:
    ...
    recommendations = search_restaurants(parsed["area"], parsed["budget"], parsed["cuisine"], meal_period, parsed.get("location_query"), parsed.get("food_query")) if slots and parsed["area"] else []
...
def model_agent(message: str) -> dict[str, Any]:
    ...
    messages: list[dict[str, Any]] = [
        {"role": "system", "content": f"你是香港课余就餐 Agent。用户学校是 {local.get('profile', {}).get('school', '未提供')}。学校和课表是可选个性化信息，不得阻止普通餐厅推荐。必须先调用 get_free_slots，再调用 search_restaurants；只要有空档就必须继续搜索餐厅，不能只回答空档时间。用户提到具体食物（例如海鲜、炸鸡、海南鸡饭、乌冬面）时，必须把它作为 food_query 传给 search_restaurants；用户说粤菜、川菜等时放入 cuisine。地点使用用户原话或已记住学校，不要把相对地点（学校旁边、校门口）原样传给搜索工具。只使用工具返回的数据，不要编造餐厅。最后用简洁中文回答，只概括空档和找到的餐厅数量，不要在文字中逐一列出餐厅、评分、价格或地址；具体餐厅列表由前端结果卡片展示。"},
            ...
            for _ in range(4):
                ...
                for call in calls:
                    ...
                    if name == "get_free_slots":
                        ...
                        tool_food_query = local["parsed"].get("food_query") or arguments.get("food_query")
                        value = search_restaurants(tool_area, int(arguments.get("budget", local["parsed"].get("budget", 100))), tool_cuisine, arguments.get("meal_period", "lunch"), tool_area, tool_food_query)
