...
def connection() -> sqlite3.Connection:
    ...
    # Migrate databases created before course locations were added.
    columns = {row["name"] for row in db.execute("PRAGMA table_info(courses)").fetchall()}
    if "location" not in columns:
        try:
            db.execute("ALTER TABLE courses ADD COLUMN location TEXT NOT NULL DEFAULT '未填写'")
        except sqlite3.OperationalError:
            # A read-only legacy DB can still be queried; writes will report a useful error later.
            pass
...
def validate_course(weekday: int, course_code: str, location: str, start_time: str, end_time: str) -> None:
    if weekday not in VALID_DAYS:
        raise ValueError("weekday 必须是 0-6（星期日到星期六）")
    if not course_code.strip():
        raise ValueError("课程代号不能为空")
    if not location.strip():
        raise ValueError("上课地点不能为空")
    try:
...
def add_course(course: dict[str, Any]) -> dict[str, Any]:
    validate_course(course["weekday"], course["course_code"], course["location"], course["start_time"], course["end_time"])
    with connection() as db:
        cursor = db.execute("INSERT INTO courses (weekday, course_code, location, start_time, end_time) VALUES (?, ?, ?, ?, ?)", (course["weekday"], course["course_code"].strip(), course["location"].strip(), course["start_time"], course["end_time"]))
...
def import_csv(csv_text: str) -> list[dict[str, Any]]:
    ...
    aliases = {"weekday": ["weekday", "星期", "星期几"], "course_code": ["course_code", "课程代号", "课程代码"], "location": ["location", "上课地点", "地点"], "start_time": ["start_time", "开始时间", "开始"], "end_time": ["end_time", "结束时间", "结束"]}
    fields: dict[str, str] = {}
    ...
    for index, row in enumerate(reader, start=2):
        ...
        try:
            ...
            item = {"weekday": weekday, "course_code": (row.get(fields["course_code"]) or "").strip(), "location": (row.get(fields["location"]) or "").strip(), "start_time": (row.get(fields["start_time"]) or "").strip(), "end_time": (row.get(fields["end_time"]) or "").strip()}
...
def courses_by_weekday(weekday: int) -> list[Course]:
    rows = list_courses()
    return [{"name": row["course_code"], "start": row["start_time"], "end": row["end_time"], "location": row.get("location", "未填写")} for row in rows if row["weekday"] == weekday]
