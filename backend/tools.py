...
def search_restaurants(area: str, budget: int = 100, cuisine: str = "all", meal_period: str = "lunch", location_query: str | None = None) -> list[dict[str, Any]]:
    live_results = search_live_restaurants(area, cuisine, location_query)
    if live_results:
        return live_results
    matches = [item for item in RESTAURANTS if item["area"] == area and item["price"] <= budget and (cuisine == "all" or cuisine in item["cuisines"])]
    return [
        {**item, "risk": item["crowd"][meal_period], "match_reason": f"{item['note']} 人均 HK${item['price']}，步行约 {item['walk_minutes']} 分钟。", "walkMinutes": item["walk_minutes"], "matchReason": f"{item['note']} 人均 HK${item['price']}，步行约 {item['walk_minutes']} 分钟。"}
        for item in sorted(matches, key=lambda item: (RISK_SCORE[item["crowd"][meal_period]], item["walk_minutes"], -item["rating"]))[:5]
    ]


def search_live_restaurants(area: str, cuisine: str = "all", location_query: str | None = None) -> list[dict[str, Any]]:
    ...
    if not api_key:
        return []
    cuisine_query = "restaurants" if cuisine == "all" else f"{cuisine} restaurants"
